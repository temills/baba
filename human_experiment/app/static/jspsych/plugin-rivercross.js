...
class jsPsychRiverCross {
  ...
  trial(display_element, trial) {
    ...
    var grid;
    if (trial.grid==null) {
        grid = generate_rand_grid(grid_rows, grid_cols + (time_limit*4), obj_types, obj_probs); // generate grid for up to 4 steps per second
    } else {
        // reverse grid so row 0 is the bottom row
        grid = []
        for (var i=trial.grid.length-1; i>=0; i--) {
            grid.push(trial.grid[i])
        }
    }
    ...
    const html = `
                ...
                <div style="position: relative; display: flex; justify-content: center;">
                    <div style="position: relative;">
                        <canvas id="game-canvas" width="` + (sqr_size*grid_cols) + `" height="` + (sqr_size*game_rows) + `" style="border:1px solid #000000;"></canvas>
                    </div>
    ...
    function get_visible_grid(grid, step) {
        ...
        for(var row=0; row<grid.length; row++) {
            var vis_row;
            if (row % 2 == 1) {
                var s = Math.floor(step/2)
                vis_row = grid[row].slice(s, s+grid_cols);
                vis_row.reverse();
            } else {
                var s = Math.ceil(step/2)
                vis_row = grid[row].slice(s, s+grid_cols);
            }
    ...
    // function get_visible_grid_prev1(grid, step) {
    //     var vis_grid = [];
    //     for(var row=0; row<grid.length; row++) {
    //         var vis_row;
    //         if (row%2 == 0) {
    //             vis_row = grid[row].slice(0, grid_cols);
    //         } else {
    //             vis_row = grid[row].slice(step, step+grid_cols);
    //             if (row % 3 == 0) {
    //                 vis_row.reverse();
    //             }
    //         }
    ...
    // function get_visible_grid_prev(grid, step) {
    //     var vis_grid = [];
    //     for(var row=0; row<grid.length; row++) {
    //         var vis_row = grid[row].slice(step, step+grid_cols);
    //         if (row % 2 == 0) {
    //             vis_row.reverse();
    //         }
    //         vis_grid.push(vis_row);
    ...
    function draw_bg() {
        ...
        ctx.fillRect(0, grid_start*sqr_size, grid_cols*sqr_size, (grid_start+game_rows)*sqr_size);
    }
    function draw_grid() {
        //ctx.fillStyle = "lightblue";
        //ctx.fillRect(0, grid_start*sqr_size, grid_cols*sqr_size, grid_rows*sqr_size);
    ...
    function update_canvas_size() {
        var max_sqr_width = window.innerWidth/grid_cols
        ...
        canvas.width = (sqr_size*grid_cols)
