...
class jsPsychRiverCross {
  ...
  trial(display_element, trial) {
    ...
    var sqr_width = sqr_size*3
    var buffer_w_prop = 3
    var buffer_h_prop = 30
    //var sqr_size = trial.sqr_size; //could have this adapt to screen size
    const speed = 20; // Pixels per second
    // Track row offsets for smooth movement
    var rowOffsets = Array(grid_rows).fill(0);
    // for (var row=0;row<grid_rows;row++) {
    //     rowOffsets[row]+= (row % 2 === 0 ? -sqr_size : 0)
    // }
    // Animation variables
    var lastTime = 0;


    // we have a grid of objects in descrete x/y positions, and avatar
    // discrete positions step every so often -- we can control this based on square size and speed
    // so we want to draw 
    // so we want some ok_buffer to the left and right of 
    // but we are drawing them as in between these positions
    // do we want to allow people to move horizontally?
    // rn we have to be below a square or to its right to move to it 
    // want it to be centered in the buffer
    // when we draw the grid, we want
    ...
    const html = `
                ...
                <div style="position: relative; display: flex; justify-content: center;">
                    <div style="position: relative;">
                        <canvas id="game-canvas" width="` + (sqr_width*grid_cols) + `" height="` + (sqr_size*game_rows) + `" style="border:1px solid #000000;"></canvas>
                    </div>
    ...
    // HANDLE ANIMATION
    function update_grid(deltaTime) {
        // Update offsets for smooth movement
        var step = false
        for (var row=0; row<grid_rows; row++){
            rowOffsets[row] += (row % 2 === 0 ? 1 : -1) * (speed * deltaTime);
            // for row 0, we then subtract sqr size
            // If a row has moved a full sqr_size, update the grid
            if (Math.abs(rowOffsets[row]) >= sqr_width) {
                rowOffsets[row]%= sqr_width;
                step = true
            }
        }
        // for row 0, top row, which moves right and is step-step+cols reversed, offsets 0-40 . for row 1, offsets 0- -40
        // row 0 is not working
        if (step) {
            step_grid()
        }
    }
    function draw_grid() {
        ...
        for (var row=0; row < vis_grid.length; row++) {
            for (var col=0; col < vis_grid[row].length; col++) {
                ...
                var true_x = col*sqr_width
                const x = true_x + offset + (row % 2 === 0 ? -sqr_width*1.5 : 0)
                const y = (row+grid_start)*sqr_size
                ctx.fillStyle = color;
                var x_buffer = (sqr_width/buffer_w_prop)
                var y_buffer = (sqr_size/buffer_h_prop)
                ctx.strokeStyle = 'black'; // Outline color
                ctx.lineWidth = 2;
                ctx.strokeRect(true_x, y, sqr_width, sqr_size);
                ctx.fillRect(x+x_buffer, y, sqr_width-(x_buffer*2), sqr_size);
                //ctx.fillRect(Math.max([x+x_buffer, 0]), y+y_buffer, sqr_size-(x_buffer*2), sqr_size-(y_buffer*2));
            }
        }
    }
    function get_visible_grid(grid, step) {
        var vis_grid = [];
        for(var row=0; row<grid.length; row++) { // one additional square in each row
            var vis_row = grid[row].slice(step, step+grid_cols+1);
            if (row % 2 == 0) {
                vis_row.reverse();
            }
    ...
    function setup_game() {
        ...
        grid = generate_rand_grid(grid_rows, grid_cols + time_limit*5, obj_types, obj_probs);
    ...
    function avatar_in_bounds() {
        return (0 <= gamestate.ypos) && (gamestate.ypos < game_rows) && (0 <= gamestate.xpos) && (gamestate.xpos < grid_cols)
    }
    ...
    function draw_bg() {
        ...
        ctx.fillRect(0, 0, grid_cols*sqr_width, grid_start*sqr_size);
        ctx.fillStyle = "#D3D3D3";
        ctx.fillRect(0, grid_start*sqr_size, grid_cols*sqr_width, (grid_start+game_rows)*sqr_size);
    }

    function draw_avatar() {
        if (avatar_in_bounds()) {
            //draw_square(avatar_color, gamestate.xpos, gamestate.ypos)
            var x = gamestate.xpos*sqr_width
            var y = gamestate.ypos*sqr_size
            var buffer;

            if (avatar_in_grid()) {
                buffer = (sqr_size/buffer_h_prop)*2
                var row = gamestate.ypos-grid_start
                var offset = rowOffsets[row];
                x += (offset + (row % 2 === 0 ? -sqr_width : 0))
            } else {
                buffer = (sqr_size/buffer_h_prop)
            }
            ctx.beginPath();
            ctx.fillStyle = avatar_color;
            //ctx.fillRect(x+x_buffer, y+y_buffer, sqr_size-(x_buffer*2), sqr_size-(y_buffer*2));
            ctx.arc(x+(sqr_width/2), y+(sqr_size/2), (sqr_size/2)-buffer, 0, Math.PI * 2);
    ...
    function update_canvas() {
        //draw background
        ctx.clearRect(0, 0, grid_cols*sqr_width, game_rows*sqr_size);
        draw_bg()
        draw_grid()
        draw_avatar()
    }
    ...
    // if we have every other square gray, then on average n_cols/2 squares per row at a time
    //
    ...
    function draw_text(text, size, color, y) {
        ...
        ctx.fillText(text, (grid_cols/2)*sqr_width - (text_width/2), y*sqr_size);
    }
    ...
    function end_game(won, reason=null) {
        ...
        ctx.globalAlpha = 0.8;
        ctx.fillStyle = "white";
        ctx.fillRect(0, 0, grid_cols*sqr_width, game_rows*sqr_size);
        ctx.globalAlpha = 1;
        if (won) {
